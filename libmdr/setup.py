"""Build script for libmdr (also the setuptools entry point).
...
def _stage_native() -> None:
    destination = Path(__file__).resolve().parent / "src" / "libmdr" / "_native"
    stage_library("mdr_shared", destination)
...
class sdist(_sdist):
    def run(self) -> None:
        # Source distributions do not embed platform binaries.
        super().run()
